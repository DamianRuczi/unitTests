...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = depts.Where(d => d.Loc == "CHICAGO")
            .Select(d => d.DeptNo)
            .Join(emps, deptNo => deptNo, emp => emp.DeptNo, (deptNo, emp) => emp)
            .ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps
            ...
            .Select(o => new { o.emp.EName, o.grade.Grade }).ToList();
    ...
    // SQL: SELECT E.EName FROM Emp E WHERE E.Sal > (SELECT AVG(Sal)
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps
            .GroupBy(e => e.DeptNo)
            .SelectMany(g => g.Where(e => e.Sal > g.Average(emp => emp.Sal)) 
                .Select(e => e.EName));
